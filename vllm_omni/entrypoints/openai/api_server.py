...
@with_cancellation
@load_aware_call
async def create_moss_ttsd_session(request: MossTTSDCreateSessionRequest, raw_request: Request):
    handler = Omnispeech(raw_request)
    if handler is None:
...
@router.post(
    "/v1/audio/speech/sessions/{session_id}/turn",
    dependencies=[Depends(validate_json_request)],
...
async def create_moss_ttsd_turn(
    ...
    if handler is None:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND.value, detail="The model does not support Speech API")
    try:
        result = await handler.create_moss_ttsd_turn(session_id, request, raw_request)
        if isinstance(result, ErrorResponse):
            return _error_response_to_json_response(result)
        return result
...
@router.delete(
    "/v1/audio/speech/sessions/{session_id}",
    status_code=HTTPStatus.NO_CONTENT.value,
)
async def delete_moss_ttsd_session(session_id: str, raw_request: Request):
    handler = Omnispeech(raw_request)
    if handler is None:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND.value, detail="The model does not support Speech API")
    try:
