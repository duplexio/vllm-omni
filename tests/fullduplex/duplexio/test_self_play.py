...
class FakeEngine:
    ...
    async def append_duplex_input_async(self, session_id: str, **kwargs: object) -> dict[str, object]:
        ...
        output = {
            ...
            "replay_prompt_frames": torch.zeros(frame_count, dtype=torch.bool),
            ...
            "user_action_eligible": torch.tensor([not final]),
            ...
            "tool_token_logprob": -torch.rand(1) if prediction else torch.empty(0),
            "tool_emit_logprob": torch.zeros(1) if prediction else torch.empty(0),
