...
def test_audio_input_graph_replays_changed_inputs(batch: int, quantized: bool) -> None:
    ...
    agent = AudioInputAdapter(32, 512, 2560).cuda().bfloat16()
    ...
    graph = AudioInputGraph(user, embedding, agent, features, codes, torch.bfloat16)
    for _ in range(3):
        features.normal_()
        if quantized:
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            expected = user(features), agent(embedding(codes))
        actual = graph(features, codes)
