...
def test_next_turn_includes_only_the_previous_turn_audio_context() -> None:
    prompt = build_realtime_prompt(
        FakeRealtimeProcessor(),
        text="next",
        reference_codes=torch.tensor([[8, 9]], dtype=torch.long),
        previous_text="prior",
        previous_codes=torch.tensor([[5, 6], [7, 8]], dtype=torch.long),
    )
...
def test_previous_text_and_audio_must_be_paired() -> None:
